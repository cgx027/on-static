...
function InventoryServiceFactory(
    ...
    InventoryService.prototype.setupDir = function setupDir(image) {
        ...
        if(!fsOp.checkPathReadable(isoFile)) {
            ...
            return db.updateImageStatus(image.name, image.version, failMessage)
                .then(function(){
            ...
            .then(function(){
                return fsOp.mountIso(isoFile, mountDir)
                    ...
                    .catch(function(err){
                        ...
                        return db.updateImageStatus(image.name, image.version, "error: Fail mounting ISO")
                            .then(function(){
                                throw new Errors.InternalServerError("error: Fail mounting ISO");
                            });
                    });
            });
    };
                    ...
                    return fsOp.downloadIso(link, targetIsoPath)
                        .catch(function(){
                            return db.updateImageStatus(
                                image.name,
                                image.version,
                                "error: Fail downloawding ISO file from web");
                        });
                    return;
                });
        } else {
    ...
    InventoryService.prototype.findImageByQuery = function findImageByQuery(query){
        return db.findImageByQuery(query);
    };
    ...
    InventoryService.prototype.addImage = function addImage(image) {
        ...
        return Promise.try(function(){
            ...
            .then(function(){
                logger.info('Adding ' + image.name + ' ' + image.version);
    ...
    InventoryService.prototype.deleteImageByQuery =
        function deleteImageByQuery(query) {
            var self = this;
                    ...
                    return Promise.each(images, function(image){
                        ...
                        return db.deleteImage(image)
                            ...
                            .then(function(){
                                ...
                                logger.info(
                                    'Image ' + image.name + ' ' + image.version + ' removed');
    ...
    InventoryService.prototype.deleteFiles = function deleteFiles(link) {
        ...
        return Promise.try(function(){
            ...
            var fileObj = constructFileObject(link);

            fsOp.removeFile(link);
