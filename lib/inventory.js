...
function InventoryServiceFactory(
    ...
    InventoryService.prototype.getOneImageByNameVersion = function getOneImageByNameVersion(
        ...
        var images = getImageByNameVersion(name, version);
        if (images.lenght > 0) return images[0];
        else return {};
    };
    ...
    InventoryService.prototype.addImage = function addImage(image) {
        ...
        var targeIsoFile = path.join(isoDirPath, nameVersionStr + '.iso');
                        ...
                        fsOp.createSymbolLink(
                            ...
                            function (err) {
                                if (err) {
                                    ...
                                    // throw new Error('Error creating symbol link from ' + source
                                    //     + ' to ' + symbolLinkDir
                                    //     + ' ' + err);
                                }
        ...
        if (fsOp.checkPathReadable(image.iso)) {
            setupDir(image.iso);
        } else if (fsOp.checkPathReadable(path.join(isoDirPath, image.iso))) {
            // try using iso file that is already downloaded to iso directory
            setupDir(path.join(isoDirPath, image.iso));
        } else if (validator.isURL(image.iso)) {
            // download iso
            fsOp.downloadIso(image.iso, targeIsoFile, function (err) {
                if (err) {
                    // remove downloaded file in case of rror
                    fs.unlinkSync(targeIsoFile)
                    self.failImageByNameVersion(image.name, image.version, 'downloading iso');
                    // throw new Error('Error downloading file from source ' + image.iso
                    //     + 'to ' + targeIsoFile,
                    //     + ' ' + err);
                } else {
                    setupDir(targeIsoFile);
                }
            });
    ...
    InventoryService.prototype.deleteImageByNameVersion = function deleteImageByNameVersion(name, version) {
        ...
        _.forEach(images, function (image) {
            if (isObjectSubset(query, image)) {
                ...
                fsOp.unmountIso(mountDir, function () {
                    fsOp.removeSymbolLink(symbolLinkDir);
                });
            }
        });
