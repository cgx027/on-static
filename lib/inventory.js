...
function InventoryServiceFactory(
    ...
    InventoryService.prototype.addImage = function addImage(source, image) {
        ...
        if (source == 'local' && fsOp.checkPathReadable(iso)) {
            ...
            var targetIsoFile, targetIsoPath;
            if (iso.endsWith('.iso')) {
                // get iso file name from http link
                var splited = iso.split('/');
                targetIsoFile = splited[splited.length - 1];
                targetIsoPath = path.join(isoDirPath, targetIsoFile);
            } else {
                targetIsoPath = path.join(isoDirPath, nameVersionStr + '.iso');
            }
            // download iso
            fsOp.downloadIso(iso, targetIsoPath, function (err) {
                if (err) {
                    // remove downloaded file in case of rror
                    fsOp.removeFile(targetIsoPath);
                    self.failImageByNameVersion(image.name, image.version, 'downloading iso');
                } else {
                    self.setupImageDir(image, targetIsoPath);
                }
            });
